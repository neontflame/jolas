...
static func get_dialogue_path(diag:String):
	var pathness:String = FileUtils.get_localized_file('res://Storystuffs/Dialogues/%s.json' % diag)
	var pathness_char:String = FileUtils.get_localized_file('res://Storystuffs/Dialogues/%s.%s.json' % [diag, GPStats.char])
...
static func get_portrait_path(character:String):
	var possibilities:Array = [
		"res://Gameplaystuffs/NPCs/%s/Portrait.tscn" % character,
...
static func get_coolname(character:String):
	var possibilities:Array = [
		"res://Gamestuffs/Gameplaystuffs/%s/DiagName.png" % character,
...
static func get_talksound_path(character:String):
	var possibilities:Array = [
		"res://Gameplaystuffs/NPCs/%s/Talksound.tres" % character,
...
static func get_voiceline_path(character:String, voiceline:String):
	var possibilities:Array = [
		"res://Gameplaystuffs/NPCs/%s/Sounds/%s" % [character, voiceline],
		GameUtils.get_char_asset_path(character, 'Sounds/' + voiceline),
		"res://Soundstuffs/Voicelines/" + voiceline
	]
