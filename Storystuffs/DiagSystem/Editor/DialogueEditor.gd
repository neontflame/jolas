...
func _on_diag_load_pressed() -> void:
	...
	var pathness = 'res://Storystuffs/Dialogues/' + $CanvasLayer/Control2/dialogueName.text + '.json'
	...
	for linery in theJayson['dialogue']:
		...
		var newThingie = load("res://Storystuffs/DiagSystem/Editor/DiagEdFragment.tscn").instantiate()
